...
opacity *= clamp((1.0 - step(intro_offset + 0.1, u_time * animation_speed_factor)) * 1.25, 1.0, 1.25);`}
      ...
      {showGradient && <div className="absolute inset-0 bg-gradient-to-t from-[#050508] to-[84%]" />}
    </div>
  );
};
...
const ShaderMaterial = ({ source, uniforms, maxFps = 60 }: { source: string; maxFps?: number; uniforms: Uniforms }) => {
  ...
  const material = useMemo(() => {
    ...
    for (const k in uniforms) {
      const u = uniforms[k];
      if (u.type === "uniform1f") prep[k] = { value: u.value };
      else if (u.type === "uniform3f") prep[k] = { value: new THREE.Vector3().fromArray(u.value as number[]) };
      else if (u.type === "uniform1fv") prep[k] = { value: u.value };
      else if (u.type === "uniform3fv") prep[k] = { value: (u.value as number[][]).map((v) => new THREE.Vector3().fromArray(v)) };
      else if (u.type === "uniform2f") prep[k] = { value: new THREE.Vector2().fromArray(u.value as number[]) };
    }
    prep.u_time = { value: 0 };
